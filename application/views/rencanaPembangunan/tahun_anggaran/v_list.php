...
function edit_tahun_anggaran(id) {
  window.location = _base_url + 'rencanaPembangunan/c_tahun_anggaran/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'rencanaPembangunan/c_tahun_anggaran/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("rencanaPembangunan/c_tahun_anggaran/delete/");?>",
					   data: "items="+itemlist,
