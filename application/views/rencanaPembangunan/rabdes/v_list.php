...
function edit_rabdes(id) {
  window.location = _base_url + 'rencanaPembangunan/c_rabdes/edit/' + id;
}

function show_rabdes_anggaran(id) {
  window.location = _base_url + 'rencanaPembangunan/c_rabdes/show_rabdes_anggaran/' + id;
}

function cetak_rabdes(id) {
  $("#MyModalBody").html('<iframe src="<?php echo base_url();?>rencanaPembangunan/c_rabdes/cetak_rabdes/'+id+'" width="100%" height="350" frameborder="no"></iframe>');
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'rencanaPembangunan/c_rabdes/add';
    }	
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("rencanaPembangunan/c_rabdes/delete/");?>",
					   data: "items="+itemlist,
  ...
  <div class="modal-dialog" style="width: 90%;">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel" ><span class="fa fa-print"></span>&nbsp;CETAK RABDes</h4>
		<h5 class="modal-title" id="myModalLabel" >&nbsp;Rencana Anggaran Belanja Desa</h5>
