...
function edit_spp(id) {
  window.location = _base_url + 'rencanaPembangunan/c_spp/edit/' + id;
}

function show_spp_detail(id) {
  window.location = _base_url + 'rencanaPembangunan/c_spp/show_spp_detail/' + id;
}

function cetak_spp(id) {
  $("#MyModalBody").html('<iframe src="<?php echo base_url();?>rencanaPembangunan/c_spp/cetak_spp/'+id+'" width="100%" height="350" frameborder="no"></iframe>');
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'rencanaPembangunan/c_spp/add';
    }	
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("rencanaPembangunan/c_spp/delete/");?>",
					   data: "items="+itemlist,
  ...
  <div class="modal-dialog" style="width: 90%;">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel" ><span class="fa fa-print"></span>&nbsp;CETAK SPP</h4>
		<h5 class="modal-title" id="myModalLabel" >&nbsp;Surat Permintaan Pembayaran</h5>
