...
function edit_periode(id) {
  window.location = _base_url + 'rencanaPembangunan/c_Periode/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'rencanaPembangunan/c_Periode/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("rencanaPembangunan/c_Periode/delete/");?>",
					   data: "items="+itemlist,
