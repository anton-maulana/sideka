...
function show_tree_rpjmdes(id) {
  window.location = _base_url + 'rencanaPembangunan/c_rkpdes/show_tree_rpjmdes/' + id;
}
...
function btn(com,grid)
{
	if (com=='Back')
    {
		window.location = _base_url + 'rencanaPembangunan/c_rkpdes/';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("rencanaPembangunan/c_rkpdes/delete_sub/");?>",
					   data: "items="+itemlist,
