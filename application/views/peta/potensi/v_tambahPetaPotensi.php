...
function startState(){
		...
		<?php 
			if($ped->lokasi != null OR $ped->lokasi != "")
			{
				echo $ped->lokasi; 
				?>
				///////////inisialiasi map lama/////////////////
				itPotensi<?= $ped->id_ped?>.setMap(null);
				polyShape = new google.maps.Polygon(polyOptionsPotensi<?= $ped->id_ped?>);
...
function polystyle() {
    ...
    this.color = "<?= $ped_sub->warna_peta?>";
    this.fill = "<?= $ped_sub->warna_peta?>";
    this.width = 1.1;
    this.lineopac = 0.7;
    this.fillopac = 0.6;
}
function logCode4(){
    ...
    gob('coords1').value = 'var myCoordinatesPotensi<?= $ped->id_ped?> = [\n';
	var arrayLoc = '[';
    ...
    if(toolID == 2){
        gob('coords1').value += '];\n';
        var options = 'var polyOptionsPotensi<?= $ped->id_ped?> = {\n'
        +'path: myCoordinatesPotensi<?= $ped->id_ped?>,\n'
        ...
        +'}\n';
        gob('coords1').value += options;
        gob('coords1').value +='var itPotensi<?= $ped->id_ped?> = new google.maps.Polygon(polyOptionsPotensi<?= $ped->id_ped?>);\n'
		+'itPotensi<?= $ped->id_ped?>.setMap(map);\n'
        +'var infoPotensi<?= $ped->id_ped?> = new google.maps.InfoWindow({content: "<b>Potensi Ekonomi Desa</b> <table><tr><td>Detil</td><td> : <?= $ped->deskripsi?></td></tr><tr><td>Jenis</td><td> : <?= $ped_sub->deskripsi?></td></tr><tr><td>Luas</td><td> : <?= $ped->luas?> Ha</td></tr></table>"});\n'
        +'google.maps.event.addListener(itPotensi<?= $ped->id_ped?>,"mouseover",function(){\n'
        +'	this.setOptions({strokeOpacity: "0.8",fillOpacity:"0.7"});\n'
		+'});\n'
		+'google.maps.event.addListener(itPotensi<?= $ped->id_ped?>,"click",function(){\n'
        +'	this.setOptions({strokeOpacity: "0.9",fillOpacity:"0.8"});\n'
        +'	infoPotensi<?= $ped->id_ped?>.open(map,itPotensi<?= $ped->id_ped?>);\n'
		+'});\n'
        +'google.maps.event.addListener(itPotensi<?= $ped->id_ped?>,"mouseout",function(){\n'
        +'	this.setOptions({strokeOpacity: "'+polygonstyles[pcur].lineopac+'",fillOpacity:"'+polygonstyles[pcur].fillopac+'"});\n'
        +'	infoPotensi<?= $ped->id_ped?>.close(map,itPotensi<?= $ped->id_ped?>);\n'
        +'});\n'
...
<div class="row">
	...
	<div class="col-md-12">
		...
		<h5 style="float:right; "><b>Detil : <?= $ped->deskripsi?> | Jenis : <?= $ped_sub->deskripsi?></b></h5>							
		<br>
		<br>
		<legend></legend>
	</div>
	...
	echo form_open_multipart('peta/c_petaPotensi/simpan_peta', $attributes); 
	?>

	<input type="hidden" value="<?= $ped->id_ped?>" name="id_ped"/>  
	<input type="hidden" value="<?= $ped->id_ped_sub?>" name="id_ped_sub"/>  

	<div class="form-group">
		<label class="col-md-0 control-label" for="simpan"></label>
		<div class="col-md-9">
			<input type="submit" value="Simpan" id="simpan" class="btn btn-success"/>
			<input type="button" value="Batal" id="batal" class="btn btn-danger" onclick="location.href='<?= base_url() ?>peta/c_petaPotensi/lists_ped/<?= $ped->id_ped_sub?>'"/>
		</div>
	</div>
	<div id="presenter">
		<form action="#">
	...
	<!--<textarea id="tempLoc" name="temp_lokasi" cols="100" rows="10"></textarea>-->
	<?php echo form_close(); ?>
