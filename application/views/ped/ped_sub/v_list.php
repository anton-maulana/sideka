...
function edit_ped_sub(id) {
  window.location = _base_url + 'ped/c_ped_sub/edit/' + id;
}
function tambah_detil_ped(id) {
  window.location = _base_url + 'ped/c_ped_sub/tambah_detil_ped/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'ped/c_ped_sub/add';
    }	
	...
	if (com=='Delete Selected Items')
        {
           ...
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("ped/c_ped_sub/delete/");?>",
					   data: "items="+itemlist,
