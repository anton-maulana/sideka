...
function edit_kepemilikan(id) {
  window.location = _base_url + 'aset/c_kepemilikan/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'aset/c_kepemilikan/add';
    }	
	...
	if (com=='Delete Selected Items')
        {
           ...
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("aset/c_kepemilikan/delete/");?>",
					   data: "items="+itemlist,
