...
function edit_aset(id) {
  window.location = _base_url + 'aset/c_aset/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'aset/c_aset/add';
    }	
	...
	if (com=='Delete Selected Items')
        {
           ...
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("aset/c_aset/delete/");?>",
					   data: "items="+itemlist,
...
<div class="col-md-4">
	...
	<li class="dropdown">
		...
		<ul class="dropdown-menu dropdown-user">
			<li>
				<a href="<?php echo site_url('aset/c_aset/ExportToExcel/');?>"><i class="fa fa-download fa-fw"></i> Export ke Excel</a>
			</li>
		</ul>
	</li>
</ul> 
