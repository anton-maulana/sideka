...
function edit_bsm(id) {
  window.location = _base_url + 'datapenduduk/c_bsm/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'datapenduduk/c_bsm/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("datapenduduk/c_bsm/delete/");?>",
					   data: "items="+itemlist,
...
function nav_active(){
	...
	var d = document.getElementById("nav-bsm");
