...
function edit_kondisi_kehamilan(id) {
  window.location = _base_url + 'datapenduduk/c_kondisi_kehamilan/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'datapenduduk/c_kondisi_kehamilan/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("datapenduduk/c_kondisi_kehamilan/delete/");?>",
					   data: "items="+itemlist,
...
<div class="col-md-4">
	...
	<li class="dropdown">
		...
		<ul class="dropdown-menu dropdown-user">
			<li>
				<a href="<?php echo site_url('datapenduduk/c_kondisi_kehamilan/ExportToExcel/');?>"><i class="fa fa-download fa-fw"></i> Export ke Excel</a>
			</li>
		</ul>
	</li>
</ul> 
